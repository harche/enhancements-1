...
* The option to set `WorkerLatencyProfile` will have to reside in a centralized location. [Recently proposed Node object](https://github.com/openshift/enhancements/blob/master/enhancements/machine-config/mco-cgroupsv2-support.md#api-extensions) in `config/v1/types_node.go` would the ideal place to define API extensions necessary for `WorkerLatencyProfile`.
...
#### Default -> MediumUpdateAndAverageReaction
...
| 20s                 | 2m           |     60s       | MediumUpdateAndAverageReaction Profile (Final state)


#### Default -> LowUpdateAndSlowReaction
...
| 1m                 | 5m           |     60s       | LowUpdateAndSlowReaction Profile (Final state)

For day-2 operation this transtion will result in **Default -> MediumUpdateAndAverageReaction -> LowUpdateAndSlowReaction** to minimize the impact on the stability of the cluster.


#### MediumUpdateAndAverageReaction -> Default
| Kubelet Status Update Frequency | KCM Node Monitor Grade Period| KAS NotReady/Unreachable Toleration | Notes |
| -----------             | -----------                              | ----------- | -----------                              |
| 20s                 | 2m           |     60s       | MediumUpdateAndAverageReaction Profile (Initial state)
| 20s                 | 2m           |     300s       | No major impact on cluster health
| 20s                 | 40s           |     60s       |  Kubelet will get only 2 chances to set the Status. **Higher chances of node incorrectly getting NotReady/Unreachable taint.**
| 20s                 | 40s           |     300s       |  Kubelet will get only 2 chances to set the Status. **Higher chances of node incorrectly getting NotReady/Unreachable taint.**
| 10s                 | 40s           |    60s       | No major impact on cluster health
...
#### MediumUpdateAndAverageReaction -> LowUpdateAndSlowReaction
| Kubelet Status Update Frequency | KCM Node Monitor Grade Period| KAS NotReady/Unreachable Toleration | Notes
| -----------             | -----------                              | ----------- | -----------                              |
| 20s                 | 2m           |     60s       | MediumUpdateAndAverageReaction Profile (Initial state)
...
| 1m                 | 5m           |    60s       | LowUpdateAndSlowReaction (Final state)


#### LowUpdateAndSlowReaction -> MediumUpdateAndAverageReaction
| Kubelet Status Update Frequency | KCM Node Monitor Grade Period| KAS NotReady/Unreachable Toleration | Notes
| -----------             | -----------                              | ----------- | ----------- |
| 1m                 | 5m           |    60s       | LowUpdateAndSlowReaction (Initial state)
| 1m                 | 2m           |     60s       | Kubelet will get only 2 chances to set the Status. **Higher chances of node incorrectly getting NotReady/Unreachable taint.**
| 20s                 | 5m           |     60s       | No major impact on cluster health
| 20s                 | 2m           |     60s       | MediumUpdateAndAverageReaction Profile (Final state)


#### LowUpdateAndSlowReaction -> Default

Because of the extremely high probability of node incorrectly getting NotReady/Unreachable taint, this transition is only allowed at day-0.

| Kubelet Status Update Frequency | KCM Node Monitor Grade Period| KAS NotReady/Unreachable Toleration | Notes
| -----------             | -----------                              | ----------- | -----------  |
| 1m                 | 5m           |     60s       | LowUpdateAndSlowReaction Profile (Initial state)
| 1m                 | 5m           |     300s       | No major impact on cluster health
| 1m                 | 40s           |     60s       |  Kubelet will NOT get any chance to set the Status. **Node will incorrectly get NotReady/Unreachable taint.**
| 1m                 | 40s           |     300s       | Kubelet will NOT get any chance to set the Status. **Node will incorrectly get NotReady/Unreachable taint.**
| 10s                 | 40s           |    60s       |    No major impact on cluster health
...
For day-2 operation this transtion will result in **LowUpdateAndSlowReaction -> MediumUpdateAndAverageReaction -> Default** to to minimize the impact on the stability of the cluster.
...
* Add CI jobs with cluster set to `MediumUpdateAndAverageReaction` and `LowUpdateAndSlowReaction` profiles
* For CI jobs with cluster set to `MediumUpdateAndAverageReaction` and `LowUpdateAndSlowReaction` profiles pass percentage is similar or better than the OpenShift current values which is nothing but `Default` latency profile.
...
2. During downgrade, if the cluster is using `MediumUpdateAndAverageReaction` or `LowUpdateAndSlowReaction` profile AND if it will get downgraded the version where it is not aware of the `WorkerLatencyProfile` then the operators MCO, KCMO, and KASO will end up eventually overriding the relevant values to their defaults.
